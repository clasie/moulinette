 ...
 //------------------------color data to generate
	...
	*/
	...
	//build the html list
        ...
        $salt = $prix['bordure_dec'][1];
        
	$html = $managercalcul->buildHtmlListElement($file_names, $file_content, 'BORDURE_DECALE_CALCULE', $salt);
        //var_dump($html); 
        //echo 'stopped here ' . __FILE__ .  ' ' . __LINE__;

	//write the html
        if($generate_new_ids)
            $filemanagercalcul->writeHtmlFile($html, 'BORDURE_DECALE_CALCULE', $concatenate_options);
	...
	*/
	...
	//build the html list                
	$css = $managercalcul->buildCssListElement($file_names, $file_content, 'BORDURE_DECALE_CALCULE', $salt);
        
	//write the html
        if($generate_new_ids){
            $filemanagercalcul->writeCssFile($css, 'BORDURE_DECALEE_CALCULE', true);
        }
        //var_dump($css);  die;
        //write list vals
        $tmp = $managercalcul->listValBuilder(array($file_names,$file_content), 'BORDURE_DECALE_CALCULE');
        //var_dump($tmp); die;
        
        //concat list vals
        $tmp = $managercalcul->concatlistValBuilder(
           array($file_content), 'BORDURE_DECALE_CALCULE', $tranche_ids['bordure_dec'][2], $tranche_ids['bordure_dec'], $tranche_ids['bordure_dec'][3]                
        );          
